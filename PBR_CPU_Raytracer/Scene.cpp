...
namespace pbr
{
    ...
    glm::vec3 Scene::FindColor(Ray& originalRay,
                               Interval& necessaryInterval,
                               int depth)
    {
        if(depth <= 0)
        {
        ...
        int intersectedInstance = 0;
        HitResult finalResult{};
        Interval currentInverval(0.0f, std::numeric_limits<float>::infinity());
        ...
        for(int i = 0; i < m_Instances.size(); i++)
        {
            ...
            Ray localRay = TransformRay(originalRay, tr.InvModelMatrix);
            HitResult localResult{};

            //TODO: move barycentric calculations, optimize normal calculation for triangle
            if(model.FindIntersection(&localRay, &localResult))
            {
                localResult.WorldSpaceIntersectinPoint = tr.ModelMatrix * glm::vec4(localResult.LocalSpaceIntersectinPoint, 1.0);
                float lenght = glm::length(originalRay.Position - localResult.WorldSpaceIntersectinPoint);
                if(lenght > currentInverval.Max)
                {
                    continue;
                }

                currentInverval.Max = lenght;
                finalResult = localResult;
                intersectionFound = true;
                intersectedInstance = i;


                //check for normal orientation
                if(glm::dot(localResult.LocalSpaceNormal, localRay.Direction) < 0)
                {
                    localResult.FrontFace = true;
                }
                else
                {
                    localResult.FrontFace = false;
                    localResult.LocalSpaceNormal *= -1;
                }
            }
        }
        ...
        {
            Instance& instance = m_Instances[intersectedInstance];
            const Transform& tr = m_Transforms[instance.transformID];

            Model& model = m_Models[instance.modelID];
            MeshPrimitive& primitive = model.Primitives[finalResult.PrimitiveID];
            Material& material = m_Materials[primitive.MaterialID];

            finalResult.WorldSpaceNormal = glm::normalize(tr.ModelMatrix * glm::vec4(finalResult.LocalSpaceNormal, 0.0f));
            finalResult.WorldSpaceIntersectinPoint = tr.ModelMatrix * glm::vec4(finalResult.LocalSpaceIntersectinPoint, 1.0);

            //return (finalResult.WorldSpaceNormal + 1.0f) / 2.0f;

            glm::vec3 randomVec = glm::normalize(glm::sphericalRand(1.0f));
            /*if(glm::dot(finalResult.WorldSpaceNormal, randomVec) < 0)
            {
                randomVec *= -1;
            }*/
            originalRay.Direction = glm::normalize(finalResult.WorldSpaceNormal + randomVec);

            /*else
            {
                originalRay.Direction = glm::reflect(originalRay.Direction, finalResult.WorldSpaceNormal);
            }*/

            originalRay.Position = finalResult.WorldSpaceIntersectinPoint + finalResult.WorldSpaceNormal * 0.01f;

            if(material.emissiveFactor.x > kEpsilon &&
               material.emissiveFactor.y > kEpsilon &&
               material.emissiveFactor.z > kEpsilon)
            {
                return material.emissiveFactor;
            }

            glm::vec3 color = glm::vec3(material.baseColorFactor) + material.emissiveFactor;
            return color * FindColor(originalRay, necessaryInterval, depth - 1);
        }
    ...
    void Scene::Render(Film* film)
    {
        ...
        auto batchProcessor = [this, film](int startRow, int endRow, int batchIndex)
        {
            ...
            float sampleStepX = xStep / SAMPLER_SQRT;
            float sampleStepY = yStep / SAMPLER_SQRT;
            ...
            for(int x = startRow; x < endRow; x++)
            {
                for(int y = 0; y < film->Height; y++)
                {
                    ...
                    for(int i = 0; i < SAMPLER_SQRT; i++)
                    {
                        for(int j = 0; j < SAMPLER_SQRT; j++)
                        {
                            float subsampleX = stepX + sampleStepX * i;
                            float subsampleY = stepY + sampleStepY * j;
                            color += GetPixelColor(subsampleX, 1.0 - subsampleY);
                        }
                    }
    ...
    {
        Ray ray = { m_Cameras[0].Position(), glm::normalize(m_Cameras[0].PixelPos(x, y) - m_Cameras[0].Position()) };

        Interval necessaryInterval = { 0.0f, std::numeric_limits<float>::infinity() };
        glm::vec3 color = FindColor(ray, necessaryInterval, PATH_DEPTH);
        return color;
    }
}